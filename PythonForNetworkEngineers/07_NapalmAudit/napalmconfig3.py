#! python3
""" napalmconfig3.py - Compares and applies ACL/OSPF configurations from a configuration
    file to a Switch
...
if len(diffs) > 0:
    print(diffs)
    iosvl2.commit_config()  # Commits the ACL changes requested by the method load_merge_candidate
else:
    print('No ACL changes required.')
    iosvl2.discard_config()  # Discards the ACL configuration loaded into the candidate
...
if len(diffs) > 0:
    print(diffs)
    iosvl2.commit_config()  # Commits the OSPF changes requested by the method load_merge_candidate
else:
    print('No OSPF changes required.')
    iosvl2.discard_config()  # Discards the OSPF configuration loaded into the candidate
